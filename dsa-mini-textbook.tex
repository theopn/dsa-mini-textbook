...
\section[Decision Tree, Lower Bound for Comparison Sorting Algo]{Decision Tree and the Lower Bound for Comparison Sorting Algorithm }
...
\begin{center}
  \begin{forest}
    ...
    [{ $a_{1}, a_{2}$ } % lv 0
      %
      [{ $a_{2}, a_{3}$ }, edge label={$\leq$} % lv 1
        [{ $[a_{1}, a_{2}, a_{3}$] }, edge label={$\leq$}] % lv 2
        [{ $a_{1}, a_{3}$ }, edge label={$>$} % lv 2
          [{ $[a_{1}, a_{3}, a_{2}$] }, edge label={$\leq$}] % lv 3
          [{ $[a_{3}, a_{1}, a_{2}$] }, edge label={$>$}] % lv 3
        ]
      ]
      %
      [{ $a_{1}, a_{3}$ }, edge label={$>$} % lv 1
        [{ $[a_{2}, a_{1}, a_{3}$] }, edge label={$\leq$}] % lv 2
        [{ $a_{2}, a_{3}$ }, edge label={$>$} % lv 2
          [{ $[a_{2}, a_{3}, a_{1}$] }, edge label={$\leq$}] % lv 3
          [{ $[a_{3}, a_{2}, a_{1}$] }, edge label={$>$}] % lv 3
        ]
      ]
    ]
  \end{forest}
...
The decision tree for array size of 3 created $3! = 6$ leaves, and it is trivial that a decision tree for an \textbf{array with $n$ elements will have $n!$ leaves}. The height of the decision tree represents the worst-case number of comparisons the algorithm has to make in order to sort the array.
...
\begin{align*}
  ...
  \geq 0 + \sum_{i = \frac{n}{2}}^{n} \log_{2}(i) \geq 0 + \sum_{i = \frac{n}{2}}^{n} \log_{2}(\frac{n}{2}) \tag{$i$ from the prev. expression starts at $\frac{n}{2}$} \\
...
\textproc{Bucket-sort} is a sorting algorithm where elements are divided into each "bucket" and a different sorting algorithm is called for each bucket. While \textproc{Bucket-sort} is a comparison sorting algorithm, it's an attempt to reduce the runtime by reducing the number of elements that the sorting algorithm has to sort.
