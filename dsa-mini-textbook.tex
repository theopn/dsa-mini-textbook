...
\begin{algorithm}
    \begin{algorithmic}[0]
      \Procedure{Bubble-sort}{$A$} \Comment{A is an array size $n$}
        \State repeat $\gets$ True
        \While{repeat is True}
          \State repeat $\gets$ False
          \For{$i = 0$ to $n - 2$}
            \If{$A[i] > A[i + 1]$}
            \State \Call{swap}{$i, i + 1$} \Comment{Assume \textproc{swap($i, j$)} is externally defined}
              \State repeat $\gets$ True
            \EndIf
          \EndFor
        \EndWhile
...
\begin{algorithm}
    \begin{algorithmic}[0]
      \Procedure{Selection-sort}{$A$} \Comment{A is an array size $n$}
        \For{$i = 0$ to $n - 2$}
        \State index $\gets$ $i$
          \For{$i = i + 1$ to $n - 1$}
            \If{$[j] < $A[index]}
